"""
Assemblage Regression for Japan Inflation — Smart Imputation Version

Uses intelligent imputation (growth rate / YoY / trend) based on each
series' characteristics before running the assemblage regression.
"""
...
def main():
    # ---- Load data --------------------------------------------------------
    print("=" * 70)
    print("ASSEMBLAGE REGRESSION — SMART IMPUTATION")
    print("=" * 70)
    ...
    print(f"\nMissing before imputation: {data_reg.isnull().sum().sum()}")

    data_reg_imputed, series_types = smart_impute(data_reg, strategy="auto",
                                                   verbose=True)
    print(f"Missing after imputation:  {data_reg_imputed.isnull().sum().sum()}")
    ...
    weights_df.to_csv(out_dir / "assemblage_weights_smart.csv", index=False)
    pd.DataFrame({
        "Date": dates,
        "Actual_12m_Forward": y,
        "Albacore_Prediction": result["fitted_values"],
    }).to_csv(out_dir / "assemblage_predictions_smart.csv", index=False)

    print("\nSaved:")
    print(f"  {out_dir / 'assemblage_weights_smart.csv'}")
    print(f"  {out_dir / 'assemblage_predictions_smart.csv'}")
    print("\nDone.")
