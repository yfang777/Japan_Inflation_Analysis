"""
regression/regression_component_no_restriction.py
  –  Albacorecomps without the non-negativity constraint

Variant of regression_component.py that:
  - Drops the w >= 0 inequality (keeps only Σw = 1).
  - Bypasses smart_impute: missing 3m/3m growth rates are filled with 0.
  - Still does a λ grid search via expanding-window CV.

    min_w  mean(y - Xw)²  +  λ ||w - w_prior||²
    s.t.   Σw = 1                              (negative weights allowed)

Run:  python regression/regression_component_no_restriction.py --level 3
"""
...
from config import (
    MIN_TRAIN, OOS_STEP, N_CV_FOLDS, ROLLING_WINDOW, PLOTS_DIR,
    START_DATE, HORIZON, COMPOSITE_COLS, SPECIAL_COLS,
)

# Override the global grid with a heavy-shrinkage sweep. The previous
# run with logspace(-4, 3, 15) saturated at λ=1000 (grid edge), so the
# true CV optimum is somewhere above 1e3. Test 1e3..1e7.
LAMBDA_GRID = np.logspace(3, 7, 9)
...
# ══════════════════════════════════════════════════════════════════════════════
#  DATA PREP (fillna 0 instead of smart_impute)
# ══════════════════════════════════════════════════════════════════════════════

def prepare_regression_data_fillna0(level: int = 3):
    """Same shape as prepare_regression_data, but fills NaN growth with 0."""
...
def _fit_single(X: np.ndarray, y: np.ndarray,
    ...
    min_w  mean(y - Xw)²  +  λ ||w - w_prior||²
    s.t.   Σw = 1                              (no non-negativity)

    Reformulated as the convex QP
        min  ½ wᵀ G w − aᵀ w        with
        G = 2 (XᵀX/n + λ I),  a = 2 (Xᵀy/n + λ w_prior)
    and solved with quadprog (active-set, dense). λ > 0 keeps G strictly
    positive definite. x0 is accepted for API compatibility but ignored.
    """
    n, k = X.shape

    G = 2.0 * (X.T @ X / n + lam * np.eye(k))
    a = 2.0 * (X.T @ y / n + lam * w_prior)
    C = np.ones((k, 1))                                # sum-to-1 only
    b = np.array([1.0])
    ...
    return {
        ...
        'n_nonzero': int((np.abs(w) > 1e-4).sum()),
        'converged': True,
    }
...
def main(level: int = 3) -> None:
    print('=' * 65)
    print(f'ALBACORECOMPS  –  No restriction, fillna(0), λ grid (level {level})')
    print('=' * 65)

    # ── data ──────────────────────────────────────────────────────────────────
    print('\n[1/5] Loading and preparing data (fillna 0)...')
    X, y, w_prior, features, dates, growth = prepare_regression_data_fillna0(level=level)
    ...
    top5_idx = np.argsort(np.abs(insample['weights']))[::-1][:5]
    print('  Top 5 |weight|:')
    for i in top5_idx:
        print(f'    {features[i]:<45}  opt={insample["weights"][i]*100:+7.2f}%'
              f'  prior={w_prior[i]*100:6.2f}%')
